...
beforeEach(async () => {
  ...
  await env.withSecurityRulesDisabled(async (ctx) => {
    ...
    await setDoc(doc(d, `groups/${GID}/repos/555`), {
      fullName: 'bob/thing', ownerUid: 'bob', registeredBy: 'bob', status: 'building',
      archived: false, v: 1,
    });
  });
});
...
describe('collabRequests', () => {
  ...
  it('requester spoof / repoOwnerUid forgery / non-pending create are denied', async () => {
    const carol = env.authenticatedContext('carol');
    await assertFails(setDoc(doc(db(carol), `groups/${GID}/collabRequests/r2`), reqDoc({ requesterUid: 'bob' })));
    await assertFails(setDoc(doc(db(carol), `groups/${GID}/collabRequests/r3`), reqDoc({ repoOwnerUid: 'carol' })));
    await assertFails(setDoc(doc(db(carol), `groups/${GID}/collabRequests/r4`), reqDoc({ state: 'accepted' })));
  });

  it('guest cannot request', async () => {
    const gia = env.authenticatedContext('gia');
    await assertFails(setDoc(doc(db(gia), `groups/${GID}/collabRequests/r5`), reqDoc({ requesterUid: 'gia' })));
  });
  ...
  it('repo owner can decide; random member cannot; admin can', async () => {
    ...
    await assertFails(updateDoc(doc(db(carol2), `groups/${GID}/collabRequests/r6`), {
      state: 'accepted', decidedBy: 'carol', decidedAt: Timestamp.now(),
    }));
    const bob = env.authenticatedContext('bob');
    await assertSucceeds(updateDoc(doc(db(bob), `groups/${GID}/collabRequests/r6`), {
      state: 'accepted', decidedBy: 'bob', decidedAt: Timestamp.now(),
    }));
  });
  ...
  it('requester may attach issue number and cancel, but not decide', async () => {
    ...
    const carol = env.authenticatedContext('carol');
    await assertSucceeds(updateDoc(doc(db(carol), `groups/${GID}/collabRequests/r7`), { githubIssueNumber: 42 }));
    await assertFails(updateDoc(doc(db(carol), `groups/${GID}/collabRequests/r7`), {
      state: 'accepted', decidedBy: 'carol', decidedAt: Timestamp.now(),
    }));
