...
describe('normalizeEvent', () => {
  ...
  it('empty push is skipped', () => {
    expect(
      normalizeEvent({ ...base, id: '2', type: 'PushEvent', payload: { distinct_size: 0 } }, REPO),
    ).toBeNull();
  });
  ...
  it('issues, release published, branch create, fork', () => {
      ...
      normalizeEvent(
        {
          ...
          id: '6',
          type: 'IssuesEvent',
          payload: { action: 'opened', issue: { number: 2, title: 'Bug', html_url: 'https://x' } },
        },
        REPO,
      ...
      normalizeEvent(
        {
          ...
          id: '7',
          type: 'ReleaseEvent',
          payload: { action: 'published', release: { tag_name: 'v1.0', html_url: 'https://r' } },
        },
        REPO,
      )!.summary,
    ).toBe('Release v1.0');
    expect(
      normalizeEvent(
        { ...base, id: '8', type: 'ReleaseEvent', payload: { action: 'created' } },
        REPO,
      ),
    ).toBeNull();
    expect(
      normalizeEvent(
        { ...base, id: '9', type: 'CreateEvent', payload: { ref_type: 'branch', ref: 'feat/x' } },
        REPO,
      )!.type,
    ).toBe('branch_created');
    expect(
      normalizeEvent(
        { ...base, id: '10', type: 'CreateEvent', payload: { ref_type: 'tag', ref: 'v1' } },
        REPO,
      ),
    ).toBeNull();
    expect(
      normalizeEvent(
        { ...base, id: '11', type: 'ForkEvent', payload: { forkee: { html_url: 'https://f' } } },
        REPO,
      )!.summary,
    ).toBe('forked by @alice');
